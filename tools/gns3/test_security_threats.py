...
Usage: python3 test_security_threats.py <target_ip> <target_mac>
"""
...
def send_telemetry(src_mac, src_ip, dst_ip="10.0.0.1", dst_port=5555):
...
def test_arp_poisoning(legit_mac, legit_ip):
    ...
    sendp(arp_req, iface="eth0", verbose=False)
    ...
    sendp(arp_mismatch, iface="eth0", verbose=False)

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 test_security_threats.py <your_legit_mac> <your_legit_ip>")
        sys.exit(1)
    ...
    print("Ensure you are running this from a guest node in the SDN network (usually iface eth0).")
    
    # 1. Legitimate traffic
    print("\n--- Testing Legitimate Traffic ---")
    send_telemetry(legit_mac, legit_ip)
    time.sleep(1)
    
    # 2. MAC Spoofing
    test_mac_spoofing(legit_mac, legit_ip)
    time.sleep(1)
    
    # 3. IP Spoofing
    test_ip_spoofing(legit_mac)
    time.sleep(1)
    
    # 4. ARP Poisoning
    test_arp_poisoning(legit_mac, legit_ip)
    time.sleep(1)
