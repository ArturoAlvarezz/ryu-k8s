...
```Ini
[Unit]
Description=Aplicar regla iptables para br0
...
Type=oneshot
RemainAfterExit=yes
# Intentará aplicar la regla. Si falla (porque br0 no existe aún), reintentará.
...
sudo systemctl enable k3s-iptables.service
sudo systemctl start k3s-iptables.service
...
sudo crontab -e
  @reboot mkdir -p /home/ubuntu/.kube && cp /etc/rancher/k3s/k3s.yaml /home/ubuntu/.kube/config && chown $(id -u ubuntu):$(id -g ubuntu) /home/ubuntu/.kube/config
kubectl get nodes
```
