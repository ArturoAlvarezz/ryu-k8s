...
- `services/security-device-registry/`: CLI registry for authorized AMI devices backed by Redis. The web UI now lives in the unified `meter-collector` dashboard on port `8081`. It registers/lists/queries/deletes devices and validates observed `mac`/`ip`/`dpid`/`in_port` tuples for future Ryu security enforcement. Worker MACs derived from switch DPID are auto-allowed and highlighted separately.
